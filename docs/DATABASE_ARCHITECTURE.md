...
> Source: _WardConnect — Backend Schema & API Specification, Version 3.0, 17 Sep 2026
> (supersedes v2.0, same day)_. This document restates that specification for the
> database layer. It does **not** add, rename or redesign anything. Where the source is
> silent, ambiguous or internally inconsistent, that is flagged in
> [§17 Source gaps and open questions](#17-source-gaps-and-open-questions) rather than
> resolved here.
...
13. [Authentication and authorization data model](#13-authentication-and-authorization-data-model)
...
18. [v1.0 → v3.0 gap analysis (traceability)](#18-v10--v30-gap-analysis-traceability)
...
| Item               | Value                                                                                                                          |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| Source document    | WardConnect — Backend Schema & API Specification v3.0 (17 Sep 2026)                                                            |
| Source supersedes  | v2.0 (same day), v1.0                                                                                                          |
| Product name       | The source calls the platform **WardConnect**; this repository is the WardSetu backend.                                        |
| Identifier policy  | All table, column, enum-value, role and endpoint names are copied verbatim from the source.                                    |
...
| Marker                          | Meaning                                                                                     |
| ------------------------------- | ------------------------------------------------------------------------------------------- |
| **NEW (v2.0)** / **NEW (v3.0)** | Table introduced in that specification version.                                             |
| **MODIFIED (v2.0)**             | Existing v1.0 table changed in v2.0.                                                        |
...
| Carried forward from v1.0 (§8)    | `users`, `auth_sessions`, `otp_challenges`, `issue_categories`, `issues`, `issue_media`, `issue_events`, `departments`, `issue_references`, `updates`, `events`, `event_rsvps`, `schemes`, `library_items`, `opportunities`, `ideas`, `idea_votes`, `polls`, `poll_options`, `poll_votes`, `report_snapshots`, `audit_logs` |     22 |
...
| **Defined in the source**         |                                                                                                                                                                                                                                                                                                                             | **42** |
| Referenced but **not defined** ⚠️ | `user_ward_roles` (role grants; see §13.3 and §17)                                                                                                                                                                                                                                                                          |      1 |
...
> modified them. The source's "22 tables carried forward" count excludes them.
...
`user_ward_roles.role` gains two values (see §13.3):
...
| Outgoing `representative_office_users` grants                                     | **Actively revoked:** `is_active = false`, `deactivated_at = now()`, and the user's `user_ward_roles` (`ward_rep_office`) row is **removed**. This is an auditable access-control action; an outgoing administration's staff must not retain live dashboard access.                         |
...
**Net effect:** "archiving" means flipping `is_current` and revoking office-staff logins, a
same-day, low-risk transaction. It **never** means bulk-copying or soft-deleting a ward's civic
...
2. Revoke that term's `representative_office_users` (as in the table above).
3. Create the new `election_terms` row and its `ward_representative_terms` row.

### 7.2 Early close — `POST /wards/:id/elections/close-term`

Explicit early close of the active term (e.g. resignation) **without** naming a successor. The ward
...
The source lists these 22 tables as carried over from v1.0 **without structural change**, except
`report_snapshots`, which gains one nullable FK in v3.0. Column types and nullability are not
...
Every table has `id UUID PK` **except** `ward_feature_flags`, which uses the composite
`PRIMARY KEY(ward_id, flag_id)`.
...
### 11.5 Indexes (source §5 "New indexes")
...
| `user_ward_roles.role`          | `citizen`, `ward_staff`, `ward_admin`, `platform_admin`, + v3.0: `ward_representative`, `ward_rep_office`             |
...
5. Revoking outgoing office users sets `is_active = false`, `deactivated_at = now()` and removes
   the matching `user_ward_roles` (`ward_rep_office`) row, all as one auditable action (§7).
...
## 13. Authentication and authorization data model
...
- **Sessions:** `auth_sessions` stores `refresh_token_hash` (never the raw token), `expires_at`
  and `revoked_at`; `POST /auth/refresh` rotates the refresh token and `POST /auth/logout`
  revokes the session.
- **Identity:** `users.mobile` is UNIQUE.

### 13.2 Roles and effective permissions (source §6)

| Role                                                                            | Access                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Citizen**                                                                     | Own issues/profile; public ward content; ideas/polls/RSVP; own consent and data-deletion requests; own device tokens.                                                                                                                                                                                                               |
| **Ward representative** (`ward_representative`, the elected person's own login) | Same effective access as Ward admin for their own ward by default (issues, analytics, notices, field activity). Publishing Updates/Events/Schemes is **recommended** to stay routed through ward staff/admin for accountability unless a ward configures otherwise. **This is a product decision, flagged rather than hard-coded.** |
| **Ward rep office** (`ward_rep_office`, PA/secretary)                           | Same effective permissions as Ward representative, **scoped to the current active `ward_representative_terms` only**; revoked automatically when that term closes (§7). Every action is audited as **"office action on behalf of [representative name]"**.                                                                          |
| **Ward staff** (`ward_staff`)                                                   | Assigned issues, verification, field notes/evidence, limited content.                                                                                                                                                                                                                                                               |
| **Ward admin** (`ward_admin`)                                                   | All ward issues/content/team/reports/settings, including running the new-term workflow (§7.1) and managing `representative_office_users` grants.                                                                                                                                                                                    |
| **Platform admin** (`platform_admin`)                                           | Cross-ward configuration and support, including states/districts/cities master data and `feature_flags`; sensitive access only when authorised and audited.                                                                                                                                                                         |

**Authorization is enforced in API middleware plus service/repository queries. Client-side
filtering is never a security control** (unchanged from v1.0).

### 13.3 `user_ward_roles` ⚠️ referenced, not defined

The source refers to `user_ward_roles.role` (§2.4), to removing a `user_ward_roles(ward_rep_office)`
row (§2.5), and to the database role enum (§0.1), but gives no column list. What can be taken
from the source:

- It grants a **role** to a **user**, and the name implies per-ward scope.
- Role values: `citizen`, `ward_staff`, `ward_admin`, `platform_admin`, `ward_representative`,
  `ward_rep_office`.
- `ward_rep_office` rows are created when office access is granted and **removed** when revoked.

Its definition must be confirmed from the v1.0 schema (or the operator's database dump) before
implementation (§17).

### 13.4 Team invitations — `invites`

`POST /team/invite` creates an invite; `GET /team/invites/:token` gives a public preview;
`POST /team/invites/:token/accept` binds the invite to the authenticated user. Only
`token_hash` is stored. Status lifecycle: `pending` → `accepted` / `expired` / `revoked`.

### 13.5 Representative office grants

Granted (`POST /wards/:id/representative/office-users`) and revoked
(`DELETE /wards/:id/representative/office-users/:userId`) by a **ward admin**; bulk-revoked when a
term closes (§7).
...
| 16.8 | **Representative publishing rights**                | Whether `ward_representative` / `ward_rep_office` may publish Updates/Events/Schemes directly is a **product decision**, currently "recommended to remain routed through ward staff/admin".                                                                                  |
...
These are **not resolved** in this document. Each needs a recorded decision (ideally a
specification revision) before the affected tables are implemented.

| #   | Gap / ambiguity                                                                                                                                                                                                                                                                                                                | Where                                 | Impact                                                                                                                                           |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **`user_ward_roles` is referenced but never defined** (columns, keys, uniqueness, whether `ward_id` is nullable for `platform_admin`/`citizen`).                                                                                                                                                                               | §2.4, §2.5, §6 of source              | Blocks the authorization model; confirm from the v1.0 schema.                                                                                    |
| 2   | **No notifications table** is listed, although in-app notifications are said to exist in v1.0.                                                                                                                                                                                                                                 | §4 of source                          | Confirm whether a v1.0 notifications table exists.                                                                                               |
| 3   | **Section 8 is missing** from the source (numbering goes §7 → §9). §11 also refers to "v1.0 (§9)".                                                                                                                                                                                                                             | Source structure                      | Confirm nothing was omitted (e.g. storage, jobs or retention sections).                                                                          |
| 4   | **Column data types are mostly unstated.** Only UUID PKs, `INT` (`term_number`, `ageing_days`), `DATE` (term dates), `BOOLEAN` (several flags) and `SMALLINT` (`rating`) are given.                                                                                                                                            | Throughout                            | Types must be decided and recorded before migrations.                                                                                            |
| 5   | **Nullability and defaults are mostly unstated** beyond explicit `NULL` markers and the `designation` default.                                                                                                                                                                                                                 | Throughout                            | Decide per column.                                                                                                                               |
| 6   | **FK targets not always stated**: `representative_office_users.added_by`, `issues.assigned_to`, `issues.locality_id`, `ideas.locality_id`, `audit_logs.ward_id` / `actor_id`, and `*_by` / `*_id` columns described only as "FK".                                                                                              | §3, §2.4                              | Confirm targets and `ON DELETE` behaviour (not specified anywhere).                                                                              |
| 7   | **Enforcement of `ward_representative_terms.ward_id = election_terms.ward_id`** is required but no mechanism is given (composite FK, trigger or application check).                                                                                                                                                            | §2.3                                  | Choose a mechanism.                                                                                                                              |
| 8   | **Status/priority value sets** for `wards.status`, `users.status`, `issues.priority`, `updates.status`, `events.status`, `event_rsvps.status`, `schemes.status`, `library_items.status`/`type`, `opportunities.status`/`audience`, `ideas.status`, `polls.status`, and `escalation_rules.escalate_to_role` are not enumerated. | §3, §4                                | Define value sets.                                                                                                                               |
| 9   | **Full v1.0 index list** is not reproduced ("all v1.0 indexes retained", one example given).                                                                                                                                                                                                                                   | §5                                    | Retrieve the v1.0 index list.                                                                                                                    |
| 10  | **Deletion/retention semantics** (what is deleted vs anonymised vs retained, retention periods).                                                                                                                                                                                                                               | §4 (`data_deletion_requests`)         | Needs a policy decision, especially given "civic data belongs to the ward".                                                                      |
| 11  | **Legacy `wards.state`/`district`/`ulb_name` trigger** behaviour on city renames or re-parenting.                                                                                                                                                                                                                              | §1                                    | Define the trigger.                                                                                                                              |
| 12  | **Product naming:** the source says "WardConnect"; this repository is "WardSetu".                                                                                                                                                                                                                                              | Title                                 | Confirm they are the same product (identifiers are unaffected).                                                                                  |
| 13  | **Issue statuses differ from the WardSetu frontend skill.** The frontend skill lists New, Acknowledged, Assigned, In Progress, Resolved, Closed, Rejected; this specification uses `submitted`, `verified`, `assigned`, `in_progress`, `resolved`, `reopened`, `rejected` and has no `closed`.                                 | Frontend skill §38 vs source §9       | **This document (the specification) is authoritative for the database.** The frontend's labels/mapping should be aligned in a separate decision. |
| 14  | **Future-module list in the backend skill** (e.g. Electoral Rolls, Voters, Polling Stations, Candidates, Campaigns) is **not part of this specification**, and the specification states that no individual's caste, religion or vote intent is stored.                                                                         | Backend skill §Purpose vs source §0.2 | Any such module needs its own specification and must respect the source's non-goals.                                                             |

---

## 18. v1.0 → v3.0 gap analysis (traceability)

Reproduced from source §10.
