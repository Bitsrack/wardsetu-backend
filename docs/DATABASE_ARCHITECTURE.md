...
> Source: _WardConnect — Backend Schema & API Specification_, **Version 6.0 (Consolidated
> Edition)**, 17 Sep 2026. v6.0 is a standalone document that merges v1.0–v5.0 and resolves six
> further open questions on roles and permissions. Earlier versions (v5.0, v4.0, v3.1, v3.0) are
> kept only for detail and rationale that v6.0 restates more briefly **without contradicting** it
> (see §1.1). Where versions differ, **v6.0 wins**. This document restates the specification for
> the database layer. It does **not** add, rename or redesign anything. Anything still silent or
> ambiguous is flagged in [§17 Source gaps and open questions](#17-source-gaps-and-open-questions).
...
18. [v1.0 → v6.0 gap analysis (traceability)](#18-v10--v60-gap-analysis-traceability)
...
| Item               | Value                                                                                                                                                             |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Source documents   | WardConnect — Backend Schema & API Specification **v6.0 (Consolidated Edition)**; earlier v5.0, v4.0, v3.1 and v3.0 for supporting detail (all dated 17 Sep 2026) |
| Current version    | **v6.0 (Consolidated Edition)**, superseding all earlier versions                                                                                                 |
...
v3.1 and v4.0 were **delta documents** that reprinted only what changed. **v5.0 and v6.0 are
consolidated, standalone editions.** Where a consolidated edition restates a table more briefly
than an earlier version (for example without the `reservation_category` value list, the legacy
`wards` text columns or the `designation` default), the earlier detail is kept here unless the
newer edition contradicts it (see §17.3).
...
| **v6.0** | **Consolidated Edition. Resolved six further open questions (gap log #24–29):** fixed a genuine self-contradiction between v5.0's worked example and its own seed list (representatives do **not** get `content:publish` by default; a ward opts in); **narrowed `ward_rep_office`'s permission set** to close a privilege-escalation path (no team/access-management or election-recording powers); defined how tiered-admin `view_analytics` **cascades down** the hierarchy while `manage_*` stays exact-tier; added the required **one-off backfill** for users predating the citizen-row rule; **split `escalation_rules:manage`** into a ward-scoped permission and a new platform-only `escalation_rules:manage_platform_defaults`; and **confirmed against the operator's real server (PostgreSQL 16.15)** that every PostgreSQL 15+ feature this spec relies on is already satisfied. |
...
- **`PUT /wards/:id/representative` was deprecated in v5.0** in favour of
  `POST /wards/:id/elections/new-term`. ⚠️ v6.0's consolidated endpoint list (§8.3) no longer
  lists it at all, but v6.0's changelog does not announce removing it. Treated here as **still
  deprecated, not confirmed removed** — see §17.3.
...
**Migration note — backfilling existing users (RESOLVED v6.0):** the auto-created `citizen` row
only applies to users created **after** this rule ships. Any user already in `users` beforehand
would otherwise have **zero `user_ward_roles` rows and zero effective permissions**, since
authorization now requires a row to resolve permissions (no implicit fallback). A **one-off,
idempotent backfill** runs as part of deploying this migration: insert a `citizen` row for every
existing `user_id` that doesn't already have one. It is safe to run more than once (a no-op for
users who already have a `citizen` row) and must run **once, immediately after the
`user_ward_roles` migration and before** application code that assumes every user has a row goes
live (gap #27). 🔶 A dry run against a production-sized dataset is recommended before the pilot
(§16.4).

`UNIQUE NULLS NOT DISTINCT` requires **PostgreSQL 15+**. The specification targets
**PostgreSQL 16.x** and, as of v6.0, this is **confirmed against the operator's real server:
PostgreSQL 16.15** — every PostgreSQL 15+ feature this spec relies on is satisfied today, not
just planned for (gap #29).
...
| ------------------ | ---- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`               | UUID |      | PK                                                                                                                                                                                                                                    |
| `ward_id`          | —    | NULL | **`NULL` = platform-wide default rule**, editable only via `escalation_rules:manage_platform_defaults` (**`platform_admin` only**, v6.0). Ward-scoped rows (`ward_id` set) are managed via `escalation_rules:manage` (own ward only). |
| `category_id`      | —    | NULL |                                                                                                                                                                                                                                       |
| `ageing_days`      | INT  |      |                                                                                                                                                                                                                                       |
...
**Privilege-escalation fix (v6.0):** before this split, `escalation_rules:manage` was granted to
ward roles with no scoping statement, so a `ward_admin` could as-written edit the platform-wide
default and change escalation behaviour for every other ward. `escalation_rules:manage` is now
**explicitly scoped to rows where `escalation_rules.ward_id` equals the caller's own ward**; only
`escalation_rules:manage_platform_defaults` (platform admin only) authorises rows where
`ward_id IS NULL`. Same pattern as `feature_flags:manage_ward` vs `feature_flags:manage_platform`.
...
| `platform`  | `wards:create`, `location_hierarchy:manage`, `feature_flags:manage_platform`, `feature_flags:manage_ward`, `escalation_rules:manage`, `escalation_rules:manage_platform_defaults` (**NEW v6.0**) |
...
| Ward representative | `issues:create`, `issues:verify`, `issues:assign`, `issues:resolve`, `issues:reject`, `issues:reopen`, `issues:link`, `issues:escalate`, `content:draft`, `ideas:moderate`, `polls:manage`, `team:invite`, `team:manage_roles`, `team:remove`, `ward:configure`, `ward:view_analytics`, `ward:manage_departments`, `ward:manage_categories`, `feature_flags:manage_ward`, `escalation_rules:manage` (own ward only), `reports:export`, `audit:view`, `elections:manage`, `representative_office:manage`. Does **not** include `content:publish` by default — **this is the confirmed, single source of truth (v6.0 fixed a contradiction in v5.0's worked example, see §13.5.6)**; a ward opts in with a `ward_role_permission_overrides` **grant**.                                                      |
| Ward rep office     | **The same set as Ward representative, MINUS `team:invite`, `team:manage_roles`, `team:remove`, `representative_office:manage` and `elections:manage`** (**narrowed in v6.0**, closing a privilege-escalation path: office staff — a PA or secretary, explicitly not the elected person — could otherwise grant themselves or a colleague higher access, remove a team member, or record an election result). Everything else (issue handling, content drafting, ward-facing operational permissions) remains, since that is the day-to-day work an office is expected to do. Term-scoped (`election_term_id` required). A ward that wants to delegate more can still do so per-person via `user_permission_overrides`, which keeps that decision individually accountable rather than a blanket default. |
...
| Platform admin      | **Every permission in the catalog, unconditionally, including `escalation_rules:manage_platform_defaults`** (v6.0).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
`ULB admin` / `District admin` / `State admin` grants also **cascade** for `view_analytics`-shaped
permissions down the location hierarchy, but not for `manage_*`-shaped ones — see §13.6.
...
- **Worked example (source, corrected in v6.0):** `role_permissions` does **not** grant
  `ward_representative` `content:publish` by default (Ward admin has it; Ward representative does
  not — §13.5.3). A ward that wants its representative to publish directly, instead of routing
  every notice through a Ward admin, inserts a `ward_role_permission_overrides` row
  `{ward_id: 12, role_key: 'ward_representative', permission_id: content:publish, effect:
'grant'}`. Now every `ward_representative` in Ward 12, including the next election's winner, can
  publish directly, until the ward removes the override. **This is an opt-in by a ward, not an
  opt-out** — consistent with the platform default being the more conservative,
  accountability-preserving choice.
  > v5.0's original worked example described the opposite (a ward using an override to **deny**
  > `content:publish`), which only makes sense if representatives had it by default — but v5.0's
  > own seed list said they didn't. v6.0 confirms the seed list was correct and rewrites the
  > example to match it (gap #24).
...
#### How tiered access reaches ward data (RESOLVED v6.0)

Two permission shapes behave differently — the distinction is the entire resolution to "does a
state/district/ULB admin see the ward-level data beneath their scope":
...
| `manage_*` (an operational or appointment action) | **Does not cascade.** It authorises acting at exactly the admin's own tier, never several levels down.                               | `district:manage_ulbs` lets a `district_admin` appoint/remove `ulb_admin` for cities in their district. It does **not** let them directly create a ward or publish ward content — that stays `ulb_admin`'s and `ward_admin`'s job, reached by appointing the right person, not by reaching past them. |

**Mechanically:** a request scoped to ward W is authorised if the caller has a matching role at
**any** level of W's ancestor chain (ward → city → district → state) whose permission is
cascading (`view_analytics`-shaped), resolved by **walking that chain** — not by an exact
scope-column match. A `manage_*`-shaped permission is only ever checked for an **exact match**
against the caller's own scope column. `wards:create` (`platform_admin`, any city) and
`ulb:manage_wards` (`ulb_admin`, their own city only) are two distinct permissions for this
reason — `POST /cities/:id/wards` accepts either, checked independently, never by walking up from
`platform_admin` to "cover" a `ulb_admin`'s narrower grant or vice versa (gap #26).

Read through `GET /{states|districts|cities}/:id/analytics` (**NEW v6.0**), which returns
aggregated analytics cascading down the hierarchy from that scope.
...
> **Cross-team item (v5.0/v6.0 §11, not a schema change):** the frontend has UI for
> State/District/ULB admin roles but **no handling yet for `ward_rep_office`**, including its
> **v6.0-narrowed permission set** (§13.5.3). That is a frontend task; the role, its permissions
> and its term scoping are fully specified here.
...
| GET      | `/{states\|districts\|cities}/:id/analytics`      | Aggregated analytics, cascading down the hierarchy from this scope (**NEW v6.0**, §13.6)                                                  |
| POST     | `/escalation-rules/platform-default`              | Create/update the platform-wide default escalation rule (`escalation_rules:manage_platform_defaults`, platform admin only) (**NEW v6.0**) |
...
| 16.4  | **Migration/restore drills**                                   | Automated migration and restore drills **before pilot** (carried over from v1.0), **including a dry run of the citizen-row backfill (§8.1) against a production-sized dataset** (v6.0).                                                                                      |
...
| 16.12 | **Frontend `ward_rep_office` support** (v5.0/v6.0 §11)         | Tracked as a **frontend** task, not a backend gap. Must reflect the **v6.0-narrowed permission set** (§13.5.3), not v5.0's broader one.                                                                                                                                      |
...
| R13 | Worked example vs seed contradicted each other on `ward_representative` publishing by default                    | **v6.0**: the example was wrong; the seed list (no default publish access) is the single source of truth, and the example was rewritten to match (§13.5.6, gap #24)                                 |
| R14 | `ward_rep_office` inherited team-management and election-recording powers with no confirmation this was intended | **v6.0**: confirmed as a genuine privilege-escalation risk and narrowed — `team:invite`/`manage_roles`/`remove`, `representative_office:manage`, `elections:manage` all excluded (§13.5.3, gap #25) |
| R15 | No stated mechanism for tiered admins to see ward data beneath them, or how access checks walk the hierarchy     | **v6.0**: `view_analytics`-shaped permissions cascade down the ancestor chain; `manage_*`-shaped permissions stay exact-tier (§13.6, gap #26)                                                       |
| R16 | Citizen-row backfill for users predating the auto-created-row rule                                               | **v6.0**: a one-off, idempotent backfill is required as part of the migration, run once before dependent application code goes live (§8.1, gap #27)                                                 |
| R17 | `escalation_rules:manage` had no scoping statement — any ward admin could edit the platform-wide default rule    | **v6.0**: split into ward-scoped `escalation_rules:manage` and platform-only `escalation_rules:manage_platform_defaults` (§9.3, §13.5.2, gap #28)                                                   |
| R18 | PostgreSQL 15+ (for `NULLS NOT DISTINCT`) was a target, not confirmed against the real server                    | **v6.0**: confirmed — the operator's server runs **PostgreSQL 16.15** (§8.1, gap #29)                                                                                                               |

### 17.2 Open — roles, permissions and `user_ward_roles`

| #   | Gap / ambiguity                                                                                                                                                                                                                                                                                                                                           | Where       | Impact                                                            |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------- |
| 1   | **Revocation mechanics differ between versions**: v3.0 removes the office user's row; v3.1/v5.0/v6.0 delete all term-scoped representative rows. v3.1/v5.0/v6.0 is authoritative.                                                                                                                                                                         | v3.0, v3.1  | None (traceability).                                              |
| 2   | **Mechanical detail of the hierarchy walk (§13.6)** — whether the ancestor chain is resolved by joining `wards.city_id → cities.district_id → districts.state_id` at query time, or by denormalising the full chain onto `user_ward_roles` / `issues` for a faster lookup, is an implementation choice the specification doesn't make.                    | v6.0 §4.2.1 | Decide during implementation; either is consistent with the spec. |
| 3   | **`ward_rep_office`'s narrowed set still includes `escalation_rules:manage`, `reports:export`, `audit:view` and `ward:configure`.** These are ward-operational rather than access/identity decisions, consistent with v6.0's stated rationale ("day-to-day work an office is expected to do"), but worth confirming none of them should also be excluded. | v6.0 §4.4   | Confirm with product if narrower still.                           |

### 17.3 Open — other

| #   | Gap / ambiguity                                                                                                                                                                                                                                                                                                                                           | Where                     | Impact                                                                                           |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------ |
| 4   | **No notifications table** is listed, although in-app notifications are said to exist in v1.0. v5.0's and v6.0's consolidated lists also have none.                                                                                                                                                                                                       | v3.0 §4, v6.0 §5–6        | Confirm whether one exists or is needed.                                                         |
| 5   | **Column data types are mostly unstated.** Only UUID PKs, `VARCHAR` (`roles.key`, `permissions.key`), `INT`, `DATE`, `BOOLEAN` and `SMALLINT` are given.                                                                                                                                                                                                  | Throughout                | Decide and record before migrations.                                                             |
| 6   | **Nullability and defaults are mostly unstated** beyond explicit `NULL` markers, the `designation` default and `roles.is_system DEFAULT true`.                                                                                                                                                                                                            | Throughout                | Decide per column.                                                                               |
| 7   | **FK targets not always stated** (`representative_office_users.added_by`, `issues.assigned_to`, `issues.locality_id`, `ideas.locality_id`, `audit_logs.ward_id` / `actor_id`, `*.created_by`), and `ON DELETE` behaviour is not specified anywhere.                                                                                                       | Throughout                | Confirm targets and delete behaviour.                                                            |
| 8   | **Enforcement of `ward_representative_terms.ward_id = election_terms.ward_id`** is required (v3.0) but no mechanism is given; v5.0/v6.0's reprint omits the "must match" note without removing it.                                                                                                                                                        | v3.0 §2.3, v6.0 §3.3      | Choose a mechanism.                                                                              |
| 9   | **Status/priority value sets** for `wards.status`, `users.status`, `issues.priority`, `updates.status`, `events.status`, `event_rsvps.status`, `schemes.status`, `library_items.status`/`type`, `opportunities.status`/`audience`, `ideas.status` and `polls.status` are not enumerated.                                                                  | Throughout                | Define value sets.                                                                               |
| 10  | **Full v1.0 index list** is not reproduced ("all original v1.0 indexes are retained").                                                                                                                                                                                                                                                                    | v6.0 §7                   | Retrieve from the v1.0 schema or the database dump.                                              |
| 11  | **Deletion/retention semantics** (what is deleted vs anonymised vs retained, retention periods).                                                                                                                                                                                                                                                          | v3.0 §4                   | Policy decision, given "civic data belongs to the ward".                                         |
| 12  | **Details v5.0/v6.0 omit without explicitly removing**: the legacy trigger-populated `wards.state`/`district`/`ulb_name` columns, the `reservation_category` value list, the `designation` default and several rationale notes. They are kept here from v3.0. Whether the consolidated editions intend to **drop the legacy text columns** is not stated. | v3.0 §1–2 vs v6.0 §2–3    | Confirm, especially the legacy columns and their trigger.                                        |
| 13  | **`PUT /wards/:id/representative` disappeared from the endpoint list in v6.0** (deprecated, not removed, in v5.0 §7.3/§8.3) with no mention in v6.0's changelog. Could be an intentional quiet removal or an accidental omission from the consolidated reprint.                                                                                           | v5.0 §8.3 vs v6.0 §8.3    | Confirm with the spec author before removing the route; treated here as still deprecated (§7.3). |
| 14  | **Product naming:** the source says "WardConnect"; this repository is "WardSetu".                                                                                                                                                                                                                                                                         | Title                     | Confirm (identifiers unaffected).                                                                |
| 15  | **Issue statuses differ from the WardSetu frontend skill** (New, Acknowledged, Assigned, In Progress, Resolved, Closed, Rejected vs `submitted`, `verified`, `assigned`, `in_progress`, `resolved`, `reopened`, `rejected`). This specification is authoritative for the database.                                                                        | Frontend skill vs v6.0 §9 | Align frontend labels/mapping.                                                                   |
| 16  | **Future-module list in the backend skill** (Electoral Rolls, Voters, Polling Stations, Candidates, Campaigns) is not in this specification. v6.0 also states: no citizen caste/religion/political preference, and **no live Election Commission feed**.                                                                                                  | Backend skill vs v6.0 §10 | Any such module needs its own specification within these non-goals.                              |

---

## 18. v1.0 → v6.0 gap analysis (traceability)

Reproduced from the source gap log (v6.0 §12, consolidating v3.0 §10, v3.1 §9, v4.0 §9, v5.0 §12).
...
| 24  | The v5.0 worked example contradicted its own seed list on whether representatives publish by default                                                                                       | Example rewritten to match the seed list: no default publish access; a ward opts IN via override (§13.5.6)                                                                                                                                           |
| 25  | `ward_rep_office` inherited team-management and election-recording powers with no confirmation this was intended                                                                           | Confirmed as a genuine risk and narrowed — `team:invite`/`manage_roles`/`remove`, `representative_office:manage`, `elections:manage` all excluded (§13.5.3)                                                                                          |
| 26  | No stated mechanism for tiered admins to see ward data beneath them, or how access checks walk the hierarchy                                                                               | `view_analytics`-shaped permissions cascade down the ancestor chain; `manage_*`-shaped permissions stay exact-tier (§13.6)                                                                                                                           |
| 27  | Existing users predating the citizen-row rule would have zero `user_ward_roles` rows and zero effective permissions                                                                        | One-off idempotent backfill required as part of this migration (§8.1)                                                                                                                                                                                |
| 28  | `escalation_rules:manage` was granted to Ward admin with no scoping — could edit the platform-wide default rule                                                                            | Split into ward-scoped `escalation_rules:manage` and platform-only `escalation_rules:manage_platform_defaults` (§9.3)                                                                                                                                |
| 29  | PostgreSQL 15+ (for `NULLS NOT DISTINCT`) was stated as a target, not confirmed against the real server                                                                                    | Confirmed: operator's server runs PostgreSQL 16.15 — every feature this spec relies on is satisfied today (§8.1)                                                                                                                                     |
...
| Escalation                              | `GET`/`POST /escalation-rules` (ward-scoped, `escalation_rules:manage`), `POST /escalation-rules/platform-default` (platform-wide default, `escalation_rules:manage_platform_defaults`, **NEW v6.0**)                                                                                                     | `escalation_rules`                                                                                                             |
...
| Tiered administration                   | `GET`/`POST /states/:id/admins`, `GET`/`POST /districts/:id/admins`, `GET`/`POST /cities/:id/admins`, `DELETE /{states\|districts\|cities}/:id/admins/:userId` (v5.0), `GET /{states\|districts\|cities}/:id/analytics` (cascading, **NEW v6.0**)                                                         | `user_ward_roles`, `audit_logs`                                                                                                |
